...
class DrinkTypeFilterTest {
    ...
    fun `apply returns true when drink category matches filter`() {
        val drink = Drink(drinkId = 1, drinkName = "Mojito", categoryName = "Cocktail", ingredients = listOf("Rum", "Lime"), measurements = listOf("1", "1"))
    ...
    fun `apply returns false when drink category does not match filter`() {
        val drink = Drink(drinkId = 2, drinkName = "Martini", categoryName = "Martini", ingredients = listOf("Vodka", "Vermouth"), measurements = listOf("1", "1"))
        val filter = DrinkTypeFilter(drinkTypeFilterValue = "Cocktail", profileId = 1)
