...
class DrinkServiceTest {
    ...
    fun setUp() {
        ...
        drinkService = DrinkService.getInstance(drinkRepository, drinkIngredientCrossRefRepository)
    }

    @Test
    fun `getInstance returns singleton instance`() {
        val instance1 = DrinkService.getInstance(drinkRepository, drinkIngredientCrossRefRepository)
        val instance2 = DrinkService.getInstance(drinkRepository, drinkIngredientCrossRefRepository)
    ...
    fun `isAllDrinkMatched returns true if all drinks are matched`() = runTest {
        ...
        val result = drinkService.isAllDrinkMatched(bypassFilter)
    ...
    private val testIngredientFilters = listOf(IngredientValueFilter("Rum", 1))
